...
class First(): # Criação da classe
    ...
    while True: # Este comando serve para iniciar uma repetição, para que o programa funcione continuamente
        ...
        cadastro_df = pd.DataFrame(data = pd.read_excel('Avaliador 360º - PBLTeX/arquivo.xlsx', engine='openpyxl'), columns=excel_header) # Aqui estamos criando um DataFrame com as informações do arquivo excel
        ...
        if window == matricula and eventos in ['-CONFIRMAR-']: # Aqui estamos especificando o que será feito quando clicar no botão 'Confirmar'
            if valores['-MATRICULA-'] and valores['-NOME-'] and valores['-SENHA-'] and valores['-TIME-'] and valores['-CARGO-'] != '': # Aqui estamos verificando se todos os campos estão preenchidos
                ...
                if salvar == True: # Aqui estamos informando que se a variavel 'salvar' for igual a 'verdadeiro' (a matrícula não se repetir) o cadastro deve ser salvo no arquivo
                    ...
                    writer = pd.ExcelWriter('Avaliador 360º - PBLTeX/arquivo.xlsx') # Aqui estamos criando a variávle responsavel por gravar as informações no arquivo excel
